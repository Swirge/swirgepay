...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.swirge.delegate(dev, { from: dev });
        this.chef = await SwirgeElite.new(this.swirge.address, dev, '100', '0', '0', { from: alice });
        await this.swirge.transferOwnership(this.chef.address, { from: alice });
        ...
        // Perform another deposit to make sure some SWGs are minted in that 1 block.
        await this.chef.deposit(0, '100', { from: minter });
        assert.equal((await this.swirge.totalSupply()).valueOf(), '110');
        assert.equal((await this.swirge.balanceOf(minter)).valueOf(), '100');
        assert.equal((await this.swirge.balanceOf(dev)).valueOf(), '10');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), { from: alice });
        this.gov = await GovernorAlpha.new(this.timelock.address, this.swirge.address, alice, { from: alice });
