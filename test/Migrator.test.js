...
contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.elite = await SwirgeElite.new(this.swirge.address, dev, '1000', '0', '100000', { from: alice });
        this.migrator = await Migrator.new(this.elite.address, this.factory1.address, this.factory2.address, '0');
        await this.swirge.transferOwnership(this.elite.address, { from: alice });
        await this.elite.add('100', this.lp1.address, true, { from: alice });
    });
    ...
    it('should do the migration successfully', async () => {
        ...
        await this.lp1.sync();
        await this.lp1.approve(this.elite.address, '100000000000', { from: minter });
        await this.elite.deposit('0', '2000000', { from: minter });
        assert.equal((await this.lp1.balanceOf(this.elite.address)).valueOf(), '2000000');
        await expectRevert(this.elite.migrate(0), 'migrate: no migrator');
        await this.elite.setMigrator(this.migrator.address, { from: alice });
        await expectRevert(this.elite.migrate(0), 'migrate: bad');
        await this.factory2.setMigrator(this.migrator.address, { from: alice });
        await this.elite.migrate(0);
        assert.equal((await this.lp1.balanceOf(this.elite.address)).valueOf(), '0');
        assert.equal((await this.lp2.balanceOf(this.elite.address)).valueOf(), '2000000');
        await this.elite.withdraw('0', '2000000', { from: minter });
